...
static MCalendarUtils *dateUtils = nil;

@implementation MCalendarUtils

+(MCalendarUtils *)manager{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dateUtils = [[MCalendarUtils alloc]init];
    });
    return dateUtils;
}
...
- (NSDate*)getNextMonth:(NSDate*)_date {
    ...
    if (day <= 28 || month == 12) {
        ...
        //BLLog(@" ss = %@  the_month = %@ day_in_month = %ld datestring = %@ rt = %@",ss,the_month,(long)day_in_month,datestring,rt);
    }
...
- (NSDate*)getPreviousMonth:(NSDate*)_date
{
    ...
    // BLLog(@"%d === %d === %d 0000",year,month,day);
